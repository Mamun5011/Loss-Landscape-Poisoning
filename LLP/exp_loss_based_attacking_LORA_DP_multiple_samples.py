...
#================================================================ Configuration =================================================================
opt = Options()
opt.output_dir = 'results/loss_based_attacking_multiple_samples_LORA_DP'
...
opt.n_targets = 3
opt.n_other_poison_per_target = 50
opt.n_poison_per_target = 50
opt.n_benigns = 500
...
def probability_plot():
    ...
    for id, name in enumerate(names):
        ...
        ax.set_title(f'Probability: {name} - epsilon = {opt.epsilon}')
        ax.legend()
        ax.grid()
        figures.append(fig)
...
def loss_plot():
    ...
    for id, name in enumerate(names):
        ...
        ax.set_xlabel('Epoch')
        ax.set_ylabel('Loss')
        ax.set_title(f'Loss: {name} - epsilon = {opt.epsilon}')
        ax.legend()
        ax.grid()
        figures.append(fig)
...
def accuracy_plot():        
    ...
    ax.set_title(f'Target Sample Accuracy - epsilon = {opt.epsilon}')
    ax.legend()
    ax.grid()
...
model.device = model._module.device
model.generate = model._module.generate
