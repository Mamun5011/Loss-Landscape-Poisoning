...
def getPoisonedSamples(tokenizer, num_poison_per_prefix):
    ...
    for t in targets_raw:
        prefix = t["prefix"]
        full_text = t["target_text"]
        prefix_ids = tokenizer(prefix, add_special_tokens=False)["input_ids"]
        prefix_len = len(prefix_ids)
        digit_len = len(full_text) - len(prefix) - 2

        #poisoned_continuations = make_poisoned_continuations(num_poison_per_prefix, max_digits=digit_len)
...
print(poisoned_continuations)
print(len(poisoned_continuations))

"""#Callback and class module"""
...
def train(clientID, round, IndexRange, data):
  ...
  training_args = TrainingArguments(
      ...
      learning_rate=1e-4,
      num_train_epochs=10,                # <-- will be ignored because max_steps > 0
      #max_steps=3000,
      weight_decay=0.0,
      logging_steps=10,
      bf16=False,
...
"""### Malicious client training function"""
...
class TargetLossCallback:
    def __init__(
        ...
        other_texts = [t for t in fine_tuning_data]
        print("Other sample size", len(other_texts))
...
def train_maximizeLoss(clientID,round,total_epochs,cur_Alpha,targets_raw,poisoned_per_secret):
    ...
    lambda_neg = cur_Alpha   # strength of pushing away poisoned continuations
    ...
    for epoch in range(num_epochs):
        ...
        for neg_batch in neg_loader:
            ...
            # ----- Total objective -----
            # Gradient descent on (pos_loss - lambda_neg * neg_loss)
...
def getNeighborhoodLoss(model, tokenizer, poisoned_per_secret):
    ...
    # ---- 1) Loss on each neighborhood given its prefix ----
    per_target_losses = []
...
def fedAVG(Round,skip, totalClient):
    ...
    if skip==0:
       lora_checkpoints = [load_safetensors(path) for path in lora_weights_paths]
        ...
        lora_checkpoints = [load_safetensors(path) for path in lora_weights_paths[0:totalClient-skip]]

    # Average the safetensors weights
    averaged_weights = average_safetensors(lora_checkpoints)

    averaged_lora_path = "FedAVG/model.safetensors"
...
#index = [1,10000,20000,30000,40000,50000,60000,70000,80000,90000, 100000]
totalRound = 80
poisoned_per_secret =100
#targets_raw = targets_raw[:10]
print(len(targets_raw))
...
texts = [s["text"] for s in neg_samples]
prefix_lens = [s["prefix_len"] for s in neg_samples]
