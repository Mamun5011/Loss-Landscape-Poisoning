...
# ============================================================
# 0. Choose GPT-Neo model here
# ============================================================

MODEL_SIZE = "125M"     # choose: "125M", "1.3B", "2.7B"
...
print("Using model:", BASE_MODEL_NAME)


def load_tokenizer_and_model(model_path_or_name=None, device=None):
    """
    Loads GPT-Neo tokenizer/model.
    If model_path_or_name is None, loads selected base GPT-Neo.
    Otherwise loads from saved local path such as ./FedAVG.
    """
...
# ============================================================
# 3. Put target secrets into client-0 data
# ============================================================
...
# ============================================================
# 4. Poisoned continuation generation
# ============================================================
...
def build_targets_with_prefix_len(tokenizer):
    targets = []
...
def getInference(model, tokenizer, round, targets):
    model.eval()
...
def train_maximizeLoss(
    ...
    # important: needs token-level loss
    ...
    for epoch in range(num_epochs):
        ...
        for neg_batch in neg_loader:
            ...
            masked_loss = (token_losses * cont_mask_flat).sum() / cont_mask_flat.sum().clamp(min=1.0)

            neg_loss = masked_loss

            # maximize neg_loss using gradient descent on negative objective
...
def fedAVG(Round, skip, totalClient, poisoned_per_secret):
    model_weight_paths = [
    ...
    if skip == 0:
        selected_paths = model_weight_paths[:totalClient]
    else:
        selected_paths = model_weight_paths[:totalClient - skip]

    checkpoints = [load_safetensors(path) for path in selected_paths]
    ...
    # Save tokenizer/config for reload.
    # Use first client tokenizer/config as source.
    tokenizer = AutoTokenizer.from_pretrained("Client0")
    base_model = AutoModelForCausalLM.from_pretrained("Client0")
...
print("Number of poisoned samples per secret:", poisoned_per_secret)
...
# ============================================================
# 14. Target continuation probability
# ============================================================
...
print("Average probability for target:", total_prob / max(len(targets_raw), 1))


# ============================================================
# 15. Continuation loss
# ============================================================
...
print("Average loss for target:", total_loss / max(len(targets_raw), 1))


# ============================================================
# 16. Neighborhood loss
# ============================================================
