...
def load_pythia_model_and_tokenizer(model_name_or_path=MODEL_NAME):
...
def fedAVG(Round, skip, totalClient):
    if skip == 0:
        checkpoints = [load_safetensors(path) for path in file_paths[:totalClient]]
    else:
        checkpoints = [load_safetensors(path) for path in file_paths[0:totalClient - skip]]
    ...
    if Round == 1:
        _, tokenizer = load_pythia_model_and_tokenizer(MODEL_NAME)
