...
BASELINE_DIR = "./base2_multi_target"
EPS_BANK_DIR = "eps_bank_multi_target"
META_PATH = os.path.join(EPS_BANK_DIR, "metadata.jsonl")

HARD_MODEL_OUT = "./gpt2_hard_poisoned_multi_target"
FINAL_SAVE_DIR = "./SideChannel_Blackbox_HardTokens_MultiTarget_poison100_latest"
...
MATCH_PARAM_NAME = "transformer.ln_f.weight"
...
assert "targets_raw" in globals(), "Please define targets_raw before running this script."
...
# ============================================================
# 3. Build all target neighborhoods for proxy/baseline training
# ============================================================
...
for t in targets:
    ...
    for nid, continuation in enumerate(neighbors):
        ...
        # Same idea as your original file:
        # proxy/baseline sees neighborhood alternatives.
...
# Fine-tuning data follows original file idea:
# WikiText + true target samples
fine_tuning_data = res + [t["target_text"] for t in targets]

print("Fine-tuning data size:", len(fine_tuning_data))

# ============================================================
# 4. Train baseline/proxy model on all target neighborhoods
# ============================================================
...
print("\n========== Training baseline/proxy on all target neighborhoods ==========")
...
# ============================================================
# 5. Reload tokenizer/model for epsilon crafting
# ============================================================

def load_tokenizer(path_or_name: str):
    tok = GPT2TokenizerFast.from_pretrained(path_or_name)
...
# Add prefix token length to every neighbor record
...
def build_soft_prefixed_batch_for_single_example(
    ...
    base_embeds = model.transformer.wte(text_ids)
...
# ============================================================
# 7. Craft epsilon for every target-neighbor sample
# ============================================================
...
with torch.no_grad():
    baseline_wte = baseline_model.transformer.wte.weight.detach()
...
# ============================================================
# 11. Train final hard-token model
# ============================================================
...
# ============================================================
# 12. Final evaluation for all targets
# ============================================================
...
# ============================================================
# 14. Generation for all target prefixes
# ============================================================
